...
angular.module('restApp').controller('BasicItemController', function($scope, $window,
        ...
        promise.then(function(response) {
            if (response.status === 200) {
                ...
                for (var i = $scope.entities.length - 1; i >= 0; i--) {
                    ...
                    if (!angular.isUndefined(entity.damages)
                    		&& angular.isUndefined(entity.damages.plus)) {
                        entity.damages.plus = 0;
                    }
                    for (var j = entity.types.length - 1; j >= 0; j--) {
	                    if (MASTER_OBJECT_LIST.includes(entity.types[j].code)) {
