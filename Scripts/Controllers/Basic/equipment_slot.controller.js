...
angular.module('restApp').controller('BasicEquipmentSlotController', function($scope, $window, basicEquipmentSlotService, $q, $http) {
    $scope.newEntity = {
        code: "",
        value: 0
    };
    var findEntity = function(entity, entities) {
        ...
        for (var i = entities.length - 1; i >= 0; i--) {
            if (entities[i].code === entity.code) {
                found = "An equipment slot with that code already exists";
                break;
            }
            if (entities[i].value === entity.value) {
                found = "An equipment slot with that number of faces already exists";
                break;
            }
        }
        return found;
    ...
    var getAllEntities = function() {
        var promise = basicEquipmentSlotService.getEntities();
    ...
    var postEntity = function() {
        var promise = basicEquipmentSlotService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = basicEquipmentSlotService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.code = "";
                $scope.newEntity.value = 0;
